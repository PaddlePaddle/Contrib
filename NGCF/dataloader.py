...
class Loader(BasicDataset):
    ...
    def __init__(self,config = config.config,path="../data/gowalla"):
        # train or test
...
#         self.testItem = self.n_user
        ...
        self.train_edge = np.concatenate([first_sub.reshape(-1, 1), second_sub.reshape(-1, 1)], axis=-1)
        self.train_edge = sorted(self.train_edge, key=lambda x : x[0])
#         self.test_edge = np.concatenate([self.testUser.reshape(-1, 1), self.testItem.reshape(-1, 1)], axis=-1)
