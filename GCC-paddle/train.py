...
def parse_option():
    ...
    parser.add_argument("--fold-idx", type=int, default=0, help="specify the fold id in 10-fold cv for choosing testing set")
    parser.add_argument("--cv", type=int, default=0, help="number of folds in the cross-validation")
    # fmt: on
...
def option_update(opt):
    fold_suffix=""
    if opt.finetune:
        fold_suffix="_fold"+str(opt.fold_idx)
    ...
    if opt.load_path is None:
        ...
        if not os.path.isdir(opt.model_folder):
            try:
                ...
                print("can not make folder:",opt.model_folder)
    else:
    ...
    if not os.path.isdir(opt.tb_folder):
        try:
            ...
            print("can not make folder:", opt.tb_folder)
    return opt
...
def clip_grad_norm(params, max_norm):
    ...
    if max_norm > 0:
        ...
        return float(np.sqrt(
            sum( np.linalg.norm(p.grad) ** 2 for p in params if p.grad is not None)
        ))
...
def main(args):
    ...
    if args.finetune:
        ...
        output_layer_optimizer = torch.optim.Adam(
            ...
            weight_decay=args.weight_decay,grad_clip=paddle.nn.clip.ClipGradByValue(max=1)
        )
    ...
    if args.optimizer == "sgd":
        ...
        if args.finetune:
            ...
            optimizer = torch.optim.Adam(
                ...
                lr=learning_rate,
                betas=(args.beta1, args.beta2),
                weight_decay=args.weight_decay,grad_clip=paddle.nn.clip.ClipGradByNorm(args.clip_norm)
            )
    ...
    if args.resume:
        if args.finetune: ##if finetune model exists, continue resume that
            if os.path.isdir(args.model_folder+"/current.pth"):
                ...
                print("change resume model to finetune model path:",args.resume)
                ##find last end epoch
                import glob
                ckpt_epoches=glob.glob(args.model_folder+"/ckpt_epoch*.pth")
                if len(ckpt_epoches)>0:
                    args.start_epoch=sorted([ int(os.path.basename(x).replace(".pth","").replace("ckpt_epoch_","")) for x in ckpt_epoches])[-1]+1
...
if __name__ == "__main__":
    ...
    if args.gpu[0]<0:
        ...
        place=paddle.CUDAPlace(args.gpu[0])
    with  paddle.fluid.dygraph.guard(place=place):
        if args.cv >0:
            ...
            if args.epochs>=5: ##speedup longer run
                f1 = Parallel(n_jobs=4,prefer="processes")(
                    delayed(main)(args) for args in variant_args_generator()
                )
            else:
                f1 = [main(args) for args in variant_args_generator()]
            cprint("10-fold Cross-validate F1:"+str(f1))
            cprint(f"Mean = {np.mean(f1)}; Std = {np.std(f1)}")
        else:
            args.gpu = args.gpu[0]
            main(args)
