...
class NodeClassificationDataset(GraphDataset):
    ...
    def _create_dgl_graph(self, data):
        ...
        num_nodes = max(dst.max().cpu().numpy().tolist()+dst.max().cpu().numpy().tolist())+ 1
...
if __name__ == "__main__":
    ...
    graph_dataset = LoadBalanceGraphDataset(
        num_workers=num_workers, aug="ns", rw_hops=4, num_neighbors=5,dgl_graphs_file="../../data/small.bin"
    )
    mem = psutil.virtual_memory()
    print(mem.used / 1024 ** 3)
