...
torch_weight = torch.load('100000_nets_ema.ckpt', map_location=torch.device('cpu'))
print(torch_weight.keys())
print(len(torch_weight['generator']))
print(len(torch_weight['mapping_network']))
print(len(torch_weight['style_encoder']))
# print(len(torch_weight['fan']))
# print(torch_weight['fan'].keys())
# for parameters in torch_weight['fan']:
#     print(parameters, ":", torch_weight['fan'][parameters].size())


# for torch_key in torch_weight['generator']['state_dict'].keys():
#     print(torch_key)

with fluid.dygraph.guard():
    generator = Generator(w_hpf=1)
    mapping_network = MappingNetwork(num_domains=2)
    style_encoder = StyleEncoder(num_domains=2)
