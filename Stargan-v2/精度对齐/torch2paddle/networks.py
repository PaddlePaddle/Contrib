    ...
    def forward(self, x_init, training=True, masks=None):
        ...
        for i in range(len(self.decode)):
            ...
            if (masks is not None) and (x.shape[2] in [32, 64, 128]):
                ...
                #mask = fluid.layers.image_resize(mask, out_shape=[x.shape[2], x.shape[3]], resample='BILINEAR')
                mask = fluid.layers.interpolate(mask, out_shape=[x.shape[2], x.shape[3]], resample='BILINEAR')
                # tmp = fluid.layers.elementwise_mul(mask, cache[x.shape[2]])
                # mask = self.hpf(tmp)
                # x = fluid.layers.elementwise_add(x , mask)  # self.hpf(mask * cache[x.shape[2]])
...
#     torch_heatmap0=np.load('/home/j/Desktop/stargan-v2-master/save_data/heatmap{}.npy'.format('0'))
#     torch_heatmap1 = np.load('/home/j/Desktop/stargan-v2-master/save_data/heatmap{}.npy'.format('1'))
#     diff0 = masks[0].numpy() - torch_heatmap0
#     #print(diff0)
#     print('diff0', np.sum(diff0))
#     diff1=masks[1].numpy()-torch_heatmap1
