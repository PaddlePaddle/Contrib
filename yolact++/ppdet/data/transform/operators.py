    ...
    def __call__(self, sample, context=None):
        ...
        if 'gt_bbox' in sample:    
            ...
            if np.sum(keep) < 1:
                ...
                sample['image'] = im
                return sample
            sample['gt_bbox'] = boxes[keep]
            gt_class = sample['gt_class']
            sample['gt_class'] = gt_class[keep]
        sample['image'] = im
        if 'gt_segm' in sample:
            if(np.sum(sample['is_crowd']) > 0):
                print('is_crowd--------------------------------------------')
            try:
                sample['gt_segm'] = masks[keep]
            except IndexError:
                sample['gt_segm'] = masks
                sample['gt_bbox'] = boxes
                sample['gt_class'] = gt_class
                print(sample['im_id'],np.sum(sample['is_crowd']))
            
        
        # print('Resize_im',np.shape(sample['image']))
        # print('Resize_segm',np.shape(sample['gt_segm']))
        # print('Resize_bbox',np.shape(sample['gt_bbox']))
        return sample
...
class ExpandImage(BaseOperator):
    def __init__(self, max_ratio, prob, mean=[123.68, 116.78, 103.94], segm_flag=False):
        """
        Args:
    ...
    def __call__(self, sample, context):
        ...
        im = sample['image']
        # print('start ExpandImage')
        # print(np.shape(im))
        # print(sample['w'],sample['h'])
        gt_bbox = sample['gt_bbox']
        gt_class = sample['gt_class']
        if self.segm_flag:
        ...
        if prob < self.prob:
            if self.max_ratio - 1 >= 0.01:
                ...
                top = random.random()*(height - im_height)
                left = random.random()*(width - im_width)
                ...
                expand_image = np.ones(
                    ...
                    dtype=im.dtype)
                # print()
                expand_image = expand_image * np.squeeze(self.mean)
                # print(height - im_height, width - im_width)
                # print('expand_image_shape',np.shape(expand_image))
                # print('im_shape',np.shape(im))
                # print('in_shape',np.shape(expand_image[int(top):int(top + im_height),
                #              int(left):int(left + im_width),:]))
                # print(im_height,im_width)
                # print(int(top),int(top + im_height),int(left),int(left + im_width))
                expand_image[int(top):int(top + im_height),
                             int(left):int(left + im_width),:] = im
                
                # print('expand_image success---------------------------------------------------')
                
                if self.segm_flag:
                    expand_masks = np.zeros(
                        (gt_segm.shape[0], height, width),
                        dtype=gt_segm.dtype)
                    expand_masks[:,int(top):int(top + im_height),
                                   int(left):int(left + im_width)] = gt_segm
                    sample['gt_segm'] = expand_masks
                    
                    # print('expand_segm success---------------------------------------------------')
                
                gt_bbox = gt_bbox.copy()
                gt_bbox[:, :2] += (int(left), int(top))
                gt_bbox[:, 2:] += (int(left), int(top))
                               
                sample['image'] = expand_image
                sample['gt_bbox'] = gt_bbox
                sample['gt_class'] = gt_class
                sample['w'] = width
                sample['h'] = height
...
class RandomCrop(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        # print('start RandomCrop')
        if 'gt_bbox' in sample and len(sample['gt_bbox']) == 0:
            print('bbox_num is zeros')
            return sample

        h = sample['h']
        w = sample['w']
        gt_bbox = sample['gt_bbox']
        # print('origin_img',np.shape(sample['image']))
        # print('origin_masks',np.shape(sample['gt_segm']))
        # print('origin_boxes',np.shape(sample['gt_bbox']))
        # print('origin_labels',np.shape(sample['gt_class']))
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                # print('crop_img',np.shape(sample['image']))
                # print('crop_masks',np.shape(sample['gt_segm']))
                # print('crop_boxes',np.shape(sample['gt_bbox']))
                # print('crop_labels',np.shape(sample['gt_class']))
                return sample
...
class PadBox(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        ...
        if 'is_crowd' in fields:
            pad_crowd = np.zeros((num_max), dtype=np.int32)
            if gt_num > 0:
                pad_crowd[:gt_num] = sample['is_crowd'][:gt_num, 0]
            sample['is_crowd'] = pad_crowd
        if 'gt_segm' in fields:
            pad_segm = np.zeros((num_max,segm.shape[1],segm.shape[2]))
            if gt_num > 0:
                pad_segm[:gt_num] = sample['gt_segm'][:gt_num, :, :]
            sample['gt_segm'] = pad_segm            
...
class Pad(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        # print('final Original image',np.shape(sample['image']))
        # print('final Original segm',np.shape(sample['gt_segm']))
        # print('final Original bbox',np.shape(sample['gt_bbox']))
        image = sample['image']
        if 'gt_segm' in sample:
            masks = sample['gt_segm']
        boxes = sample['gt_bbox']
        depth, im_h, im_w = image.shape
        
        expand_image = np.zeros(
            (self.height, self.width, depth),
            dtype=image.dtype)
        expand_image[:, :, :] = self.mean
        expand_image = np.transpose(expand_image, [2, 0, 1])
        ...
        if 'gt_segm' in sample:
            sample['gt_segm'] = expand_masks
        # print('final image',np.shape(sample['image']))
        # print('final segm',np.shape(sample['gt_segm']))
        # print('final bbox',np.shape(sample['gt_bbox']))
        # print('final label',np.shape(sample['gt_class']))
        # print('final crowd',np.shape(sample['is_crowd']))
# ['image', 'im_info', 'im_id', 'gt_bbox', 'gt_class', 'is_crowd', 'gt_segm']
        return sample

import random   
@register_op
class RandomMirror(BaseOperator):
    def __init__(self):
...
@register_op
class ToPercentCoords(BaseOperator):
    """
    Convert bbox XYXY format to XYWH format.
    """
    ...
    def __call__(self, sample, context=None):
        ...
        # num = sample['gt_num']
        # print(num)
        # x1 = sample['gt_bbox'][:num,0]
        # print(x1)
        # y1 = sample['gt_bbox'][:num,1]
        # print(y1)
        # x2 = sample['gt_bbox'][:num,2]
        # print(x2)
        # y2 = sample['gt_bbox'][:num,3]
        # print(y2)
        # b = sample['image']
        # b = np.transpose(b,(1,2,0))
        # for i in range(num):
        #     b1 = cv2.rectangle(b, (int(x1[i]), int(y1[i])), (int(x2[i]), int(y2[i])), (255, 255, 255))
        #     print(x1[i], y1[i], x2[i], y2[i])
        #     plt.imsave('b' + str(i) + '.jpg',sample['gt_segm'][i].astype('float32'))
        #     cv2.imwrite('t' + str(i) + '.jpg', b1)
        # plt.imsave('b.jpg',sample['gt_segm'][num].astype('float32'))
        # print('finish')
        # sys.exit()
        # print(x1,x2,x3,x4)
        sample['gt_bbox'][:, 0] /= w
        ...
        sample['gt_bbox'][:, 1] /= h
        sample['gt_bbox'][:, 3] /= h
        # plt.imsave('a.jpg',sample['image'])
        # print(np.max(sample['image']))

        return sample

@register_op        
class PrepareMasks(BaseOperator):
    """
    ...
    def __init__(self, mask_size=16):
        super(PrepareMasks, self).__init__()
    ...
    def __call__(self, sample, context=None):
        ...
        new_masks = np.zeros((masks.shape[0], self.mask_size, self.mask_size))

        for i in range(len(masks)):
            x1, y1, x2, y2 = boxes[i, :]
            x1, y1, x2, y2 = (int(x1), int(y1), int(x2), int(y2))
            if x1 == 0 and x2 == 0 and y1 == 0 and y2 == 0:
                # print('PrepareMasks finish')
                break
            ...
            y2 *= height
            boxes[i, :] = [x1, y1, x2, y2]
            new_masks[i, :] = scaled_mask
        
        # Binarize
        new_masks[new_masks >  0.5] = 1
        new_masks[new_masks <= 0.5] = 0
        sample['gt_bbox'] = boxes
        sample['gt_segm'] = new_masks
        print('PrepareMasks',np.shape(new_masks))
        return sample
