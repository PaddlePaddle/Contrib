...
class Reader(object):
    ...
    def _load_batch(self):
        ...
        while bs != self._batch_size:
            ...
            if self._load_segm:
                # print('_load_segm----------------------------------------------------------------------------')
                ins_anno_ids = self._dataset.datasettype.getAnnIds(imgIds=sample['im_id'], iscrowd=False)
                # print(ins_anno_ids)
                instances = self._dataset.datasettype.loadAnns(ins_anno_ids)
    
                if len(instances) > 0:
                    # print('build_masks---------------------------------------------------------')
                    # Pool all the masks for this image into one [num_objects,height,width] matrix
                    masks = [self._dataset.datasettype.annToMask(obj).reshape(-1) for obj in instances]
                    masks = np.vstack(masks)
                    masks = masks.reshape(-1, int(sample['h']), int(sample['w']))
                else:
                    masks = None
