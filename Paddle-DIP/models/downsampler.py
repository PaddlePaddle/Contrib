...
class Downsampler(nn.Layer):
    ...
    def __init__(self, n_planes, factor, kernel_type, phase=0, kernel_width=None, support=None, sigma=None, preserve_size=False):
        ...
        downsampler.weight[:] = 0
        downsampler.bias[:] = 0

        # kernel_tensor = paddle.to_tensor(self.kernel, dtype='float32', stop_gradient=False)
        for i in range(n_planes):
            downsampler.weight[i, i] = self.kernel
        ...
        if preserve_size:
            ...
            self.padding = nn.Pad2D(pad, mode='replicate')
