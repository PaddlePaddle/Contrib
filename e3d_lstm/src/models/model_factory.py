...
class Model(object):
  ...
  def load(self, checkpoint_path):
    ...
    print(self.gen_ims.name)
    # frozen_graph = self.freeze_session(self.sess, output_names=[self.gen_ims.name])
    # frozen_graph = self.freeze_session(self.sess, output_names=['strided_slice_2'])
    # tf.train.write_graph(frozen_graph, "frozen", "frozen.pb", as_text=False)
