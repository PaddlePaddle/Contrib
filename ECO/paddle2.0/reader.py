...
class ECO_Dataset(IterableDataset):

    def __init__(self, name, cfg, mode):

        super(ECO_Dataset, self).__init__()
        self.cfg = cfg
        self.mode = mode
        self.name = name
            ...
            with open(self.filelist) as flist:
                ...
                for line in lines:
                    ...
                    if len(self.batch_out) == self.batch_size:
                        ...
                        self.batch_out = []
        for data in _reader():
            dy_x_data = np.array([x[0] for x in data])
            y_data = np.array([[x[1]] for x in data])
            yield dy_x_data, y_data
